...
local LIBRARY_URL = "https://raw.githubusercontent.com/habibrodriguez7-art/FreemiumLib/refs/heads/main/lib10.lua"
local Lynx = nil
...
CombinedModules.blatantv1 = (function()
    ...
    NetEvents.RE_MinigameChanged.OnClientEvent:Connect(function(state)
...
function UnlimitedZoomModule.Disable()
    ...
    Player.CameraMinZoomDistance = 0.5
    Player.CameraMaxZoomDistance = 128 -- Default Roblox limit
...
local function Cast()
    ...
    pcall(function()
        NetEvents.RF_ChargeFishingRod:InvokeServer({[22] = tick()})
...
local function setupListeners()
    ...
    MinigameConnection = NetEvents.RE_MinigameChanged.OnClientEvent:Connect(function(state)
    ...
    FishCaughtConnection = NetEvents.RE_FishCaught.OnClientEvent:Connect(function(fishName, data)
        if not BlatantAutoFishing.Enabled then return end
...
CombinedModules.BuildGUI = function(Lynx)
    ...
    if CombinedModules.TeleportModule then
        ...
        Title = "Select Location",
            ...
            Default = nil,
            Callback = function(selected)
                CombinedModules.TeleportModule.TeleportTo(selected)
            end
        })
    end
    ...
    PlayerTPSection:AddDropdown({
        ...
        Callback = function(selected)
            if CombinedModules.TeleportToPlayer then
                CombinedModules.TeleportToPlayer.TeleportTo(selected)
            end
        end
    })
   ...
   local EventTPSection = TeleportTab:AddSection("Event Teleport", false)
    ...
    EventTPSection:AddDropdown({
        ...
        Callback = function(selected)
            ...
            if autoEventToggle and autoEventToggle:GetValue() then
    ...
    SellSection:AddInput({
        ...
        Callback = function(value)
            local v = tonumber(value)
            if v and CombinedModules.AutoSellSystem and CombinedModules.AutoSellSystem.Timer then
    ...
    SellSection:AddToggle({
        ...
        Default = false,
        Callback = function(on)
            if CombinedModules.AutoSellSystem and CombinedModules.AutoSellSystem.Timer then
                if on then CombinedModules.AutoSellSystem.Timer.Start() else CombinedModules.AutoSellSystem.Timer.Stop() end
            end
        end
    })
    ...
    -- Section: Auto Sell By Count
    local CountSection = ShopTab:AddSection("Auto Sell By Count", false)
    
    CountSection:AddInput({
        Title = "Target Fish Count",
        Default = "235",
        Callback = function(value)
            local v = tonumber(value)
            if v and CombinedModules.AutoSellSystem and CombinedModules.AutoSellSystem.Count then
                CombinedModules.AutoSellSystem.Count.SetTarget(v)
            end
        end
    })
    
    CountSection:AddToggle({
        Title = "Enable Auto Sell By Count",
        Default = false,
        Callback = function(on)
            if CombinedModules.AutoSellSystem and CombinedModules.AutoSellSystem.Count then
                if on then CombinedModules.AutoSellSystem.Count.Start() else CombinedModules.AutoSellSystem.Count.Stop() end
            end
        end
    })
    ...
    PerformanceSection:AddDropdown({
        ...
        Default = "60",
        Callback = function(value)
            local fps = tonumber(value) or 60
            if CombinedModules.UnlockFPS then CombinedModules.UnlockFPS.SetCap(fps) end
        end
    })
